// Vanilla port of frontend/components/game/core/useRendererBootstrap.ts
// The mount effect becomes bootstrapRenderer() returning a dispose function.
...
export type RendererBootstrapOptions = {
    isMounted: boolean;
...
export function bootstrapRenderer(options: RendererBootstrapOptions): () => void {
    ...
    {
        if (!options.isMounted || !options.canvasRef.current) {
            return () => undefined;
        }
        ...
        const initialize = async () => {
            try {
                ...
                const debugOverlayVisible = options.showDebugOverlay ?? false;
                ...
                fpsText.y = 8;
                fpsText.zIndex = 1000;
                fpsText.visible = debugOverlayVisible;
                ...
                pingText.visible = debugOverlayVisible;
