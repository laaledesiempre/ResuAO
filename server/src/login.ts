...
const DEFAULT_WELCOME_CONSOLE_MESSAGES = [
    "Bienvenido a Resu. Si quieres enterarte de las últimas actualizaciones del juego, puedes ingresar a nuestro Discord.",
...
// Welcome lines come from the instance site_config (messages.serverWelcome,
// editable from the admin UI) with a short cache so edits apply without a
// restart. Falls back to the defaults when unset or unreachable.
const WELCOME_MESSAGES_CACHE_MS = 60_000;
let welcomeMessagesCache: { fetchedAt: number; messages: string[] } | null = null;

async function getWelcomeConsoleMessages(): Promise<string[]> {
    if (welcomeMessagesCache && Date.now() - welcomeMessagesCache.fetchedAt < WELCOME_MESSAGES_CACHE_MS) {
        return welcomeMessagesCache.messages;
    }

    let messages = DEFAULT_WELCOME_CONSOLE_MESSAGES;

    try {
        const result = (await funct.fetchUrl("/api/site-config", {
            headers: { Authorization: vars.tokenAuth },
        })) as { config?: { messages?: { serverWelcome?: unknown } } | null };
        const raw = result?.config?.messages?.serverWelcome;

        if (Array.isArray(raw)) {
            const lines = raw.filter((line): line is string => typeof line === "string" && line.trim().length > 0);
            if (lines.length > 0) {
                messages = lines;
            }
        }
    ...
    welcomeMessagesCache = { fetchedAt: Date.now(), messages };
    return messages;
}
...
function sendWelcomeConsoleMessage(client: RuntimeClient, messages: string[]) {
