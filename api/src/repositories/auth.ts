...
import { getRegistrationSettings } from "./siteConfig";
import type {
    AccountRecord,
...
function toPublicSession(
    ...
    return {
        account: {
            _id: account.id,
            name: account.name,
            // Los placeholders de email automaticos no se exponen al cliente.
            email: account.email?.endsWith("@placeholder.local")
                ? null
                : account.email,
...
export async function registerAccount(payload: unknown): Promise<AuthResponse> {
    ...
    if (!registration.enabled) {
        throw new Error("El registro de cuentas esta deshabilitado.");
    }

    const { name, email, password } = registerSchema.parse(payload);

    if (registration.requireEmail && !email) {
        throw new Error("El email es obligatorio");
    }

    const nameSanitized = sanitizeName(name);
    // Cuando el email no es obligatorio y no se provee, guardamos un hash
    // placeholder unico para mantener la constraint UNIQUE de la columna.
    const normalizedEmail = email
        ? normalizeEmail(email)
        : `noemail-${crypto.createHash("sha256").update(`${nameSanitized}:${crypto.randomBytes(16).toString("hex")}`).digest("hex")}@placeholder.local`;
    ...
    if (account && resetToken) {
        try {
            await sendPasswordResetEmail({
                to: account.email ?? normalizedEmail,
...
export async function consumeGameTicket(
    ...
    | {
          account: { _id: string; name: string; email: string | null };
          character: Awaited<
